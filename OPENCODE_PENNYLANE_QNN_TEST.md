# OpenCodeAI Prompt: PennyLane QNN Classifier Benchmark

**AUTO-EXECUTION MODE - Complete PennyLane LRET Plugin Validation**

## Mission

Run a complete automated benchmark comparing LRET PennyLane plugin against PennyLane's default.mixed device for QNN (Quantum Neural Network) classifier. Test 8 and 10 qubits, measure performance, validate results, and generate a comprehensive analysis report.

## Execution Requirements

**Autonomy Level**: FULL AUTO  
**Expected Duration**: 2-6 hours  
**Prerequisites**: None - install everything automatically  
**Output**: Complete markdown report with performance comparison

## Step-by-Step Instructions (Execute Automatically)

### Phase 1: Environment Setup (Auto-Install Everything)
...
2. **Install required packages** (if not present)
   ```bash
   pip install pennylane>=0.30 torch numpy scipy psutil memory_profiler matplotlib pandas
   ```

3. **Build LRET Python bindings**
   ```bash
   cd /Users/suryanshsingh/Documents/LRET/python
   pip install -e . --force-reinstall
   ```

4. **Verify LRET device loads**
   ```bash
   python -c "import pennylane as qml; dev = qml.device('qlret.mixed', wires=4, epsilon=1e-4); print('✓ LRET device loaded:', dev)"
   ```
   
   **If this fails:**
   - Check that `python/qlret/build/` exists and has .so/.dylib files
   - Rebuild: `cd python && rm -rf build/ && pip install -e .`
   - Verify again
...
5. **Create results directory**
   ```bash
   mkdir -p /Users/suryanshsingh/Documents/LRET/results/qnn_benchmark
   cd /Users/suryanshsingh/Documents/LRET/results/qnn_benchmark
   ```

6. **Create benchmark script**  
   Save the following to `qnn_benchmark.py`:
...
7. **Run the benchmark**
   ```bash
   cd /Users/suryanshsingh/Documents/LRET/results/qnn_benchmark
   python qnn_benchmark.py
   ```
   
   **Expected duration**: 2-6 hours total  
   **Output**: Will print progress and save results automatically

### Phase 4: Verify Results

8. **Check outputs exist**
   ```bash
   ls -lh /Users/suryanshsingh/Documents/LRET/results/qnn_benchmark/
   # Should show:
   # - qnn_benchmark_results.json
   # - QNN_BENCHMARK_REPORT.md
   ```

9. **Display final report**
   ```bash
   cat /Users/suryanshsingh/Documents/LRET/results/qnn_benchmark/QNN_BENCHMARK_REPORT.md
   ```

### Phase 5: Analysis & Conclusion

10. **Analyze results** - Check that:
    - ✓ LRET device completed training without errors
    - ✓ Training accuracy is reasonable (>50% for this toy problem)
    - ✓ Performance metrics match expectations:
      - At 8 qubits: Comparable or baseline slightly faster (expected)
      - At 10 qubits: LRET should show 1.2-2× speedup or memory advantage
    - ✓ Final report contains comparison tables and analysis

11. **Determine success**  
    Benchmark is **successful** if:
    - All 4 tests completed (LRET + baseline for 8q and 10q)
    - LRET accuracy matches baseline (within 10%)
    - LRET shows performance advantage at 10 qubits OR comparable performance
    - No crashes or OOM errors

## Expected Results

**At 8 qubits** (baseline territory):
- Time: LRET ~45-60s, baseline ~40-55s (comparable)
- Memory: LRET ~280-400 MB, baseline ~300-450 MB (comparable)
- Accuracy: Both ~60-80% (toy problem)

**At 10 qubits** (entering LRET advantage):
- Time: LRET ~150-200s, baseline ~200-280s (LRET 1.2-1.5× faster)
- Memory: LRET ~600-800 MB, baseline ~2-3 GB (LRET 3-4× less memory!)
- Accuracy: Both ~60-80% (should match)

**Success Criteria:**
✓ LRET completes all tests  
✓ Accuracy within 10% of baseline  
✓ Shows memory advantage at 10 qubits (2-4× less)  
✓ Speed comparable or faster at 10 qubits  

## Failure Handling

**If LRET device fails to load:**
- Rebuild bindings: `cd python && pip install -e . --force-reinstall`
- Check CMake build: `cd build && make -j$(sysctl -n hw.ncpu)`
- Verify .so files exist: `ls python/qlret/build/`

**If benchmark crashes:**
- Check available memory: `python -c "import psutil; print(f'{psutil.virtual_memory().available/(1024**3):.1f} GB free')"`
- Reduce to 8 qubits only if memory limited
- Check error logs in traceback

**If comparison looks wrong:**
- Verify both devices used same circuit (same seed)
- Check that epochs/batch_size match
- Review raw JSON for anomalies

## Output Files to Review

1. **qnn_benchmark_results.json** - Raw data
2. **QNN_BENCHMARK_REPORT.md** - Analysis report (THIS IS THE MAIN DELIVERABLE)

## Final Action

**Report the final summary from QNN_BENCHMARK_REPORT.md in your response.**  
Include:
- Performance table
- Speedup/memory comparison for each qubit count
- Overall conclusion
- Whether LRET plugin is working correctly

---

## Summary

This prompt runs an automated 2-6 hour benchmark testing LRET PennyLane plugin for QNN classifier at 8 and 10 qubits. It:
1. Auto-installs dependencies
2. Builds LRET Python bindings
3. Runs training on both LRET and default.mixed
4. Compares performance (time, memory, accuracy)
5. Generates comprehensive markdown report

**Run this in OpenCodeAI terminal:**
```bash
# From LRET root directory
mkdir -p results/qnn_benchmark
cd results/qnn_benchmark
# Copy the benchmark script above to qnn_benchmark.py
python qnn_benchmark.py
```

**Then review QNN_BENCHMARK_REPORT.md for results.**
