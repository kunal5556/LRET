/**
 * LRET GPU Core Functionality Test - Kaggle Edition
 *
...
// Error checking (from your actual code)
...
public:
    ...
    bool distribute_state(const std::vector<std::complex<double>>& L_host,
    ...
    bool gather_state(std::vector<std::complex<double>>& L_out) {
        if (!d_L_) {
            std::cerr << "ERROR: No data on device" << std::endl;
            return false;
        }
...
int main() {
    ...
    try {
        ...
        bool success = gpu_sim.distribute_state(L_input, rows, cols);
        if (!success) {
            std::cerr << "✗ distribute_state failed!" << std::endl;
            return 1;
        }
        ...
        success = gpu_sim.gather_state(L_output);
        if (!success) {
            std::cerr << "✗ gather_state failed!" << std::endl;
            return 1;
        }
