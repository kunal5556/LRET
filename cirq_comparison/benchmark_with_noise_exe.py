"""
Comprehensive Benchmark with Noise: LRET vs Cirq (using quantum_sim.exe)
- Qubits: 7-9
- Depth: 15  
- Noise: 0.1% depolarizing per gate
- Uses quantum_sim.exe binary directly to get latest DEPOLARIZE support
"""
import time
import numpy as np
import json
...
# Output directory
OUTPUT_DIR = Path("d:/LRET/cirq_comparison/benchmark_results_with_noise_exe")
...
log("="*80)
log("COMPREHENSIVE BENCHMARK WITH NOISE: LRET vs Cirq (using quantum_sim.exe)")
...
N_TRIALS = 3
QUANTUM_SIM_EXE = "d:/LRET/build/Release/quantum_sim.exe"

def simulate_lret(circuit_dict):
    """Simulate using quantum_sim.exe."""
    ...
    try:
        result = subprocess.run(
            [QUANTUM_SIM_EXE, "--input-json", input_file],
            capture_output=True,
            text=True,
            timeout=300
        )
        ...
        if result.returncode != 0:
            return {"status": "error", "final_rank": None, "output": output}
        
        try:
            result_json = json.loads(output)
            return {
                "status": result_json.get("status", "success"),
                "final_rank": result_json.get("final_rank", 1),
...
for n_qubits in QUBITS:
    ...
    for trial in range(N_TRIALS):
        ...
        lret_result = simulate_lret(lret_circuit)
