#!/usr/bin/env python3
"""
LRET Parallel Modes Comparison Benchmark - QUICK VERSION
=========================================================
...
QUICK VERSION Configuration (optimized for faster results):
- Qubits: [8,12,16,20,24,28,32] (7 points vs 12 - fewer intermediate points)
- Depth: 15 CNOT layers (vs 25 - 40% fewer gates)
- Noise: 0.5% depolarizing per gate (vs 1% - less rank growth)
- Epsilon: 1e-5 (vs 1e-6 - faster SVD)
- Trials: 2 (vs 3 - 33% fewer runs)
- Total: 84 runs vs 216 runs (2.6× reduction)

Estimated time reduction: 4-6× faster than full version
"""
...
if not IS_WORKER:
    ...
    print("=" * 80)
    print("LRET PARALLEL MODES COMPARISON BENCHMARK - QUICK VERSION")
    print("=" * 80)
    print(f"Platform: {platform.system()}")
    print(f"Script: {os.path.basename(script_path)}")
    print(f"Results directory: {log_dir}")
    print("")
    print("QUICK VERSION SETTINGS:")
    print("  - Qubits: [8,12,16,20,24,28,32] (7 points)")
    print("  - Depth: 15 layers | Noise: 0.5% | Epsilon: 1e-5 | Trials: 2")
    print("  - Total: 84 runs (vs 216 in full version)")
    print("  - Expected time: ~4-6× faster")
    print("")
    ...
    launch_in_new_terminal(benchmark_cmd, "LRET Parallel Modes Benchmark - QUICK")
...
# =============================================================================
CONFIG = {
    'qubits': [8, 12, 16, 20, 24, 28, 32],   # 7 points (fewer intermediate values)
    'depth': 15,                             # 15 CNOT layers (40% fewer gates)
    'noise_prob': 0.005,                     # 0.5% depolarizing (less rank growth)
    'epsilon': 1e-5,                         # Relaxed truncation (faster SVD)
    'n_trials': 2,                           # 2 trials (33% fewer runs)
}

# Total configurations: 7 qubits × 6 modes × 2 trials = 84 runs (vs 216 in full version)
...
def main():
    ...
    log("=" * 80)
    log("LRET PARALLEL MODES COMPARISON BENCHMARK - QUICK VERSION")
    log("=" * 80)
    log(f"LRET Root: {LRET_ROOT}")
    log(f"quantum_sim.exe: {QUANTUM_SIM}")
    log(f"Output directory: {LOG_DIR}")
    log(f"")
    log(f"Configuration (QUICK VERSION - Optimized for Speed):")
