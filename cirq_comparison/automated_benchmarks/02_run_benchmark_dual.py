#!/usr/bin/env python3
"""
LRET Automated Benchmark Runner (Dual Terminal Version)
=========================================================

Same as 02_run_benchmark.py but with dual terminal support:
- Opens a window for benchmark output
- Opens a window for CPU monitoring
...
# =============================================================================
if not IS_WORKER:
    from parallel_modes_comparison.launcher_utils import launch_in_new_terminal, get_terminal_name, format_command_for_platform
    ...
    log_dir = os.path.join(script_dir, f'results_{run_id}')
    os.makedirs(log_dir, exist_ok=True)
    
    terminal_name = get_terminal_name()
    
    print("=" * 70)
    print("LRET vs CIRQ AUTOMATED BENCHMARK (Dual Terminal)")
    print("=" * 70)
    ...
    launch_in_new_terminal(benchmark_cmd, "LRET vs Cirq Benchmark")
    
    # Wait a moment for benchmark to start
    time.sleep(2)
    
    # Start CPU monitor in new window with log_dir argument
    monitor_path = os.path.join(script_dir, "parallel_modes_comparison", "monitor_cpu.py")
...
def main():
    ...
    log("="*70)
    log("LRET AUTOMATED BENCHMARK (Dual Terminal Version)")
    log("="*70)
