"""
Comprehensive Benchmark: LRET vs Cirq with Noise
- Qubits: 7-11
- Depth: 10
- Noise: 0.1% depolarizing per layer
...
print("="*80)
print("COMPREHENSIVE BENCHMARK: LRET vs Cirq with Noise")
print("Configuration: 7-11 qubits, depth=10, noise=0.1% per layer")
print("="*80)
...
QUBITS = list(range(7, 12))  # 7, 8, 9, 10, 11
DEPTH = 10
NOISE_PER_GATE = 0.001  # 0.1%
N_TRIALS = 3

def build_circuit_lret(n_qubits, depth, noise_prob):
    """Build circuit for LRET with noise after each gate."""
    ops = []
    ...
    for i in range(n_qubits):
        ...
        if noise_prob > 0:
            ops.append({"name": "DEPOLARIZE", "wires": [i], "parameter": noise_prob})
    ...
    for d in range(depth):
        ...
        for i in range(0, n_qubits - 1, 2):
            ...
            if noise_prob > 0:
                ops.append({"name": "DEPOLARIZE", "wires": [i], "parameter": noise_prob})
                ops.append({"name": "DEPOLARIZE", "wires": [i+1], "parameter": noise_prob})
...
def compute_fidelity(rho1, rho2):
    ...
    # Simplified fidelity: Tr(rho1 @ rho2)
    return np.abs(np.trace(rho1 @ rho2)).real

def run_benchmark_single(n_qubits, depth, noise_prob):
    ...
    for trial in range(N_TRIALS):
        ...
        if trial == 0:
            ...
            if result.get('status') == 'success' and 'state' in result:
                ...
                print(f"    Warning: LRET state not exported (status={result.get('status')})")
    ...
    if result['fidelity'] is not None:
        ...
        print(f"  Fidelity: N/A (state export failed)")
...
for n_qubits in QUBITS:
    try:
        result = run_benchmark_single(n_qubits, DEPTH, NOISE_PER_GATE)
...
with open(csv_path, 'w') as f:
    f.write("n_qubits,depth,noise_prob,lret_wall_mean,lret_wall_std,lret_internal_mean,")
    f.write("cirq_mean,cirq_std,speedup,final_rank,fidelity,trace_distance,max_diff\n")
    
    for r in results:
        f.write(f"{r['n_qubits']},{r['depth']},{r['noise_prob']},{r['lret_wall_mean']:.6f},")
...
if HAS_MATPLOTLIB and len(results) > 0:
    ...
    ax.set_title(f'Execution Time: LRET vs Cirq (depth={DEPTH}, noise={NOISE_PER_GATE*100:.1f}%)', fontsize=14)
    ...
    ax.set_title(f'LRET Speedup over Cirq (depth={DEPTH}, noise={NOISE_PER_GATE*100:.1f}%)', fontsize=14)
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title(f'LRET Rank Evolution (depth={DEPTH}, noise={NOISE_PER_GATE*100:.1f}%)', fontsize=14)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title(f'State Fidelity: LRET vs Cirq (depth={DEPTH}, noise={NOISE_PER_GATE*100:.1f}%)', fontsize=14)
    ax.grid(True, alpha=0.3)
    ...
    # Plot 5: Trace distance
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.semilogy(qubits, trace_dists, marker='o', linewidth=2, markersize=8, color='red')
    ax.set_xlabel('Number of Qubits', fontsize=12)
    ax.set_ylabel('Trace Distance', fontsize=12)
    ax.set_title(f'Trace Distance: LRET vs Cirq (depth={DEPTH}, noise={NOISE_PER_GATE*100:.1f}%)', fontsize=14)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plot5_path = OUTPUT_DIR / "plot_trace_distance.png"
    plt.savefig(plot5_path, dpi=150)
    print(f"  Saved: {plot5_path}")
    plt.close()
    
    # Plot 6: Combined summary (2x2 grid)
    ...
    fig.suptitle(f'Comprehensive Benchmark: LRET vs Cirq (depth={DEPTH}, noise={NOISE_PER_GATE*100:.1f}%)', 
                 fontsize=16, y=0.995)
    plt.tight_layout()
    plot6_path = OUTPUT_DIR / "plot_summary.png"
...
if results:
    ...
    print(f"  Depth: {DEPTH}")
    print(f"  Noise: {NOISE_PER_GATE*100:.2f}% per gate")
