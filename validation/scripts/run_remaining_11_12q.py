#!/usr/bin/env python3
"""
Run remaining 11-12q benchmarks that weren't captured in partial results.
"""
...
def main():
    ...
    output_file = base_dir / "results" / "phase_e_11_12q_results.json"
    
    # Load already completed circuits
    completed = set()
    if partial_file.exists():
        with open(partial_file, 'r') as f:
            data = json.load(f)
            for r in data.get("results", []):
                completed.add(r["circuit"])
    ...
    for i, circuit_path in enumerate(remaining):
        try:
            ...
            print(f"[{i+1}/{len(remaining)}] {circuit_path.name}: Failed to load metadata: {e}")
            continue
        
        n_qubits = meta["n_qubits"]
        n_ops = meta["n_operations"]
        
        # Longer timeout for larger circuits
        if n_ops > 400:
            timeout = 900  # 15 min
        elif n_ops > 200:
            timeout = 600  # 10 min
        ...
        print(f"[{i+1}/{len(remaining)}] {circuit_path.name[:55]:55} | {n_qubits}q, {n_ops} ops, T={timeout}s", end="", flush=True)
    ...
    print()
    print("=" * 80)
    print(f"COMPLETE: {len(results)} circuits benchmarked, {len(errors)} errors")
    print("=" * 80)
