...
/**
 * @brief Compute trace of L L† using row-parallel reduction
 * 
 * OPTIMIZATION (Phase 1): Row-parallel trace computation.
 * Computes Tr[ρ] = Tr[L L†] = ||L||_F² = Σᵢⱼ |L(i,j)|² 
 * using OpenMP parallel reduction over rows for better cache efficiency.
 * 
 * @param L Low-rank factor
 * @return Trace of the density matrix (should be 1.0 for normalized states)
 */
double compute_trace_L_parallel(const MatrixXcd& L);

/**
 * @brief Compute diagonal probabilities using row-parallel access
 * 
 * OPTIMIZATION (Phase 1): Row-parallel probability computation.
 * For measurement sampling, we need P(i) = ρ(i,i) = Σⱼ |L(i,j)|²
 * This function computes all diagonal elements efficiently using
 * row-parallel access patterns.
 * 
 * @param L Low-rank factor
 * @return Vector of diagonal probabilities P(i) for each basis state
 */
VectorXd compute_diagonal_probabilities_parallel(const MatrixXcd& L);

/**
 * @brief Sample measurement outcome using row-parallel probabilities
 * 
 * OPTIMIZATION (Phase 1): Combines parallel probability computation
 * with efficient binary search sampling.
 * 
 * @param L Low-rank factor
 * @param seed Random seed (0 for time-based)
 * @return Sampled basis state index
 */
size_t sample_measurement_parallel(const MatrixXcd& L, unsigned int seed = 0);

/**
 * @brief Compute expectation value of Pauli Z on specified qubit
 * 
 * OPTIMIZATION (Phase 1): Row-parallel expectation value computation.
 * <Z_q> = Σᵢ (-1)^bit(i,q) * P(i) where P(i) = Σⱼ |L(i,j)|²
 * 
 * @param L Low-rank factor
 * @param qubit Qubit index for Z measurement
 * @param num_qubits Total number of qubits
