...
// SIMD pragma compatibility: MSVC requires -openmp:experimental for omp simd
// GCC/Clang support it with standard -fopenmp
#if defined(_OPENMP) && !defined(_MSC_VER)
    #define OMP_SIMD_PRAGMA _Pragma("omp simd")
    #define OMP_SIMD_REDUCTION(op, var) _Pragma("omp simd reduction(" #op ":" #var ")")
#else
    // On MSVC or without OpenMP, skip SIMD pragmas (let compiler auto-vectorize)
    #define OMP_SIMD_PRAGMA
    #define OMP_SIMD_REDUCTION(op, var)
#endif
...
//==============================================================================
// Row-Parallel Utilities (Phase 1 Optimization)
//==============================================================================

double compute_trace_L_parallel(const MatrixXcd& L) {
    // Compute Tr[L L†] = ||L||_F² = Σᵢⱼ |L(i,j)|²
    // Using row-parallel reduction for cache efficiency
    
    size_t dim = L.rows();
    size_t rank = L.cols();
    
    // For small matrices, use Eigen's optimized squaredNorm
    if (dim * rank < 4096) {
        return L.squaredNorm();
    }
    ...
    for (int64_t i = 0; i < idim; ++i) {
        double row_sum = 0.0;
        // SIMD-friendly inner loop over columns (rank)
        // Note: On GCC/Clang this uses omp simd, on MSVC relies on auto-vectorization
        for (int64_t j = 0; j < irank; ++j) {
            row_sum += std::norm(L(i, j));  // |L(i,j)|²
        }
        total += row_sum;
    }
...
VectorXd compute_diagonal_probabilities_parallel(const MatrixXcd& L) {
    // Compute P(i) = ρ(i,i) = Σⱼ |L(i,j)|² for all basis states
    // This is the probability of measuring each computational basis state
    ...
    for (int64_t i = 0; i < idim; ++i) {
        double row_sum = 0.0;
        // SIMD-friendly inner loop over rank dimension
        for (int64_t j = 0; j < irank; ++j) {
            row_sum += std::norm(L(i, j));
        }
        probs(i) = row_sum;
    }
...
size_t sample_measurement_parallel(const MatrixXcd& L, unsigned int seed) {
    // Compute probabilities in parallel
    VectorXd probs = compute_diagonal_probabilities_parallel(L);
    
    // Normalize if necessary (should already be ~1.0 for valid states)
    ...
    // Binary search through cumulative distribution
    // First compute cumulative probabilities
    size_t dim = probs.size();
    double cumsum = 0.0;
    ...
    return dim - 1;  // Fallback for numerical edge case
}

double compute_pauli_z_expectation_parallel(const MatrixXcd& L, size_t qubit, size_t num_qubits) {
    // <Z_q> = Σᵢ (-1)^bit(i,q) * P(i) where P(i) = Σⱼ |L(i,j)|²
    // = P(qubit=0) - P(qubit=1)
    ...
    double prob_0 = 0.0;  // Probability of qubit being 0
    double prob_1 = 0.0;  // Probability of qubit being 1
    
    int64_t idim = static_cast<int64_t>(dim);
    int64_t irank = static_cast<int64_t>(rank);
    
#ifdef _OPENMP
    #pragma omp parallel for reduction(+:prob_0, prob_1) schedule(static) if(dim > 256)
#endif
    for (int64_t i = 0; i < idim; ++i) {
        double row_prob = 0.0;
        // SIMD-friendly inner loop
        for (int64_t j = 0; j < irank; ++j) {
            row_prob += std::norm(L(i, j));
        }
        
        // Check if qubit is 0 or 1 in basis state i
        if ((i & qubit_mask) == 0) {
            prob_0 += row_prob;
        } else {
            prob_1 += row_prob;
