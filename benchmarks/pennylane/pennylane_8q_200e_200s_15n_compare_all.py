...
=========================================================
Configuration: 8 qubits, 200 epochs, 200 samples, 15% noise (HEAVY)

Tests all parallelization modes:
- LRET: sequential, row, column, hybrid, batch
- Baseline: default.mixed (expected to OOM)
...
if not IS_WORKER:
    ...
    print("⚠️  WARNING: This is a HEAVY benchmark (8q, 200 epochs)")
    print("=" * 70)
...
# =============================================================================
print("=" * 80)
print("COMPREHENSIVE COMPARISON: ALL LRET MODES + default.mixed (HEAVY)")
print("=" * 80)
...
print(f"\n⚠️  WARNING: HEAVY benchmark - default.mixed WILL likely OOM")
...
# Test default.mixed (expected to OOM at this scale)
print(f"\n>>> Testing default.mixed (EXPECTED TO OOM) <<<")
try:
    ...
    # Only try 5 epochs for baseline at this scale
    results["default_mixed"] = train("default.mixed", circuit_baseline, init_params.copy(), max_epochs=5)
...
# =============================================================================
print("\n" + "=" * 80)
print("COMPREHENSIVE COMPARISON RESULTS (HEAVY BENCHMARK)")
print("=" * 80)
print(f"\n{'Device':<20} {'Status':<10} {'Total Time':>12} {'Time/Epoch':>12}")
print("-" * 60)

sorted_results = sorted(results.items(), 
                       key=lambda x: x[1]["total_time_seconds"] if x[1].get("status") != "OOM" else float('inf'))

for name, r in sorted_results:
    status = r.get("status", "completed")
    if status == "OOM":
        print(f"{name.upper():<20} {'OOM':<10} {'N/A':>12} {'N/A':>12}")
    else:
        print(f"{name.upper():<20} {'OK':<10} {r['total_time_seconds']:>10.1f}s "
              f"{r['avg_epoch_time']:>10.2f}s")
...
if lret_results:
    ...
    print(f"\n✓ FASTEST LRET MODE: {best_name.upper()} ({best_result['total_time_seconds']:.1f}s)")
    if results["default_mixed"].get("status") == "OOM":
        print(f"✓ default.mixed: OUT OF MEMORY - LRET is the ONLY option at this scale!")
