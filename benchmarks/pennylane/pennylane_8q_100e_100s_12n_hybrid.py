#!/usr/bin/env python3
"""
LRET vs default.mixed Training Benchmark - HYBRID Parallelization (8 Qubits)
=============================================================================
Configuration:
  - Qubits: 8
  - Epochs: 100
...
Expected runtime: LRET ~10-20 minutes, default.mixed ~30-60 minutes (may OOM)

WARNING: 8-qubit density matrices are large. default.mixed may run out of memory.
...
if len(sys.argv) <= 1 or sys.argv[1] != "--worker":
    ...
    print("Mode: HYBRID parallelization (8 qubits)")
    print(f"Results directory: {log_dir}")
    print("⚠️  WARNING: default.mixed may OOM at 8 qubits!")
    print("=" * 70)
    
    benchmark_cmd = f'cd "{script_dir}"; python "{script_path}" --worker "{log_dir}"'
    subprocess.Popen(["powershell", "-NoExit", "-Command", benchmark_cmd], creationflags=subprocess.CREATE_NEW_CONSOLE)
    time.sleep(2)
    monitor_path = os.path.join(script_dir, "monitor_cpu.py")
    monitor_cmd = f'cd "{script_dir}"; python "{monitor_path}" "{log_dir}"'
    subprocess.Popen(["powershell", "-NoExit", "-Command", monitor_cmd], creationflags=subprocess.CREATE_NEW_CONSOLE)
...
print("=" * 70)
print("LRET vs default.mixed TRAINING BENCHMARK (8 QUBITS)")
print("=" * 70)
